...
class SimpleScreenshotWrapper extends StatefulWidget {
  final Widget child;
  final String? screenName;

  const SimpleScreenshotWrapper({
    super.key,
    required this.child,
    this.screenName,
  });
  ...
  @override
  State<SimpleScreenshotWrapper> createState() =>
      _SimpleScreenshotWrapperState();
}

class _SimpleScreenshotWrapperState extends State<SimpleScreenshotWrapper> {
    ...
    try {
      ...
      if (image != null) {
        ...
        if (await file.exists() && await file.length() > 0) {
          _showMessage(
            'Đã lưu ảnh chụp màn hình thành công!\nVị trí: ${file.path}',
          );
        } else {
